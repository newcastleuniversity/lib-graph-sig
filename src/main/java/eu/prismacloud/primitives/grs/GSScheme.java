...
import eu.prismacloud.primitives.grs.parameters.*;
...
//        gs_encoding_params.setLabelEncodingLength(16);
        ...
        GSGraphEncodingResult graphEncodingResult = graphEncoding.setup(keyGenPair.getPrivateKey(), keyGenPair.getPrivateKey(), keyGenPair, keyGenPair.getSignature());
...
//        ICommitment recCommitment = recMessage.getCommitment();
        ...
        signer.hiddenSign((GSCommitment) recipientCommitment, signerConnectingVertex, recipientConnectingVertex, graphEncoding.getExtendedPublicKey(), signerGraph, graphEncoding.getExtendedPrivateKey());
//
...
//        GSGraphSignature signerPreGSignature = signerMessage.getSignature();
        ...
        IGraphSignature signResult = recipient.hiddenSign(recipientCommitment, signerConnectingVertex, recipientConnectingVertex, graphEncoding.getExtendedPublicKey(), recipientGraph, rndRecipient);

        //Verify
        IVerifier verifier = new GSVerifier();
        BigInteger rndVerifier = Utils.computeRandomNumberSymmetric(KeyGenParameters.l_0.getValue());
        Boolean result = recipient.verify(graphEncoding.getExtendedPublicKey(), recipientCommitment, rndRecipient, signResult.getGraphSignature());
