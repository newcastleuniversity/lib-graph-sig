...
/**
 * Created by Ioannis Sfyrakis on 27/01/2022
 */
public class VerifierOrchestratorBC implements IVerifierOrchestrator {
	...
	public void init() throws IOException {
		...
		this.n_3 = verifier.computeNonce();

		messageElements = new HashMap<URN, Object>();
		messageElements.put(URN.createZkpgsURN("verifier.n_3"), this.n_3);
	...
	private BaseCollection constructBaseCollection(Map<URN, Object> proofSignatureElements) {
		...
		Iterator<Map.Entry<URN, Object>> candidateEntries = proofSignatureElements.entrySet().iterator();
...
//		}
	...
	private void computeCommitmentVerifiers() throws VerificationException, ProofStoreException {
		...
		for (BaseRepresentation vertex : vertexIterator) {
			...
			BaseRepresentation base = new BaseRepresentation(extendedPublicKey.getPublicKey().getBaseR(), -1, BaseRepresentation.BASE.BASER);
...
//		for (Map.Entry<URN, GSCommitment> commitmentEntry : C_i.entrySet()) {
...
//		}
	...
	public void executeVerification() throws VerificationException, NoSuchAlgorithmException, ProofStoreException {
