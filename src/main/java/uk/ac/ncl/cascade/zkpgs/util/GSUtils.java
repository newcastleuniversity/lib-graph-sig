...
public class GSUtils implements INumberUtils {
	 ...
	 * @return the list
	 */
	public List<BigInteger> splitHexString(final String str, final int chunkLength) {
		List<BigInteger> splitArray = new ArrayList<>();

		String element;
		for (int i = 0; i < str.length(); i += chunkLength) {
			element = str.substring(i, Math.min(str.length(), i + chunkLength)).toUpperCase();
			splitArray.add(new BigInteger(element, 16));
		}
	 ...
	 * @return the string
	 */
	public BigInteger joinHexString(final List<BigInteger> splitArray) {
		String binString = "";

		for (BigInteger numb : splitArray) {
			binString = binString.concat(numb.toString(16).toUpperCase());
		}

		return new BigInteger(binString, 16);
	}
}
