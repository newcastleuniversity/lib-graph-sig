...
class GSUtilsTest {
	...
	void splitHexString() {
		...
		List<String> strings = classUnderTest.splitHexString(hexString, 128);
	...
	void joinHexString() {
		...
		List<String> strings = classUnderTest.splitHexString(binString, 32);
		log.info("strings length: " + strings.size());
		List<BigInteger> bigArray = new ArrayList<BigInteger>();

		for (int i = 0; i < strings.size(); i++) {
			log.info("index: " + i + " st: " + strings.get(i));
			bigArray.add(new BigInteger(strings.get(i), 16));
		}
		...
		for (int i = 0; i < bigArray.size(); i++) {
			...
			BigInteger conv = new BigInteger(strings.get(i), 16);
		...
		List<String> stringsFromBigInteger = new ArrayList<String>();

		for (BigInteger bigInteger : bigArray) {

			stringsFromBigInteger.add(bigInteger.toString(16).toUpperCase());
		}

		String str = classUnderTest.joinHexString(stringsFromBigInteger);
