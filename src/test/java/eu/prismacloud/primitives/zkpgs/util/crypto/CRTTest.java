...
class CRTTest {
  ...
  void computeCRTRandomExp() {
    ...
    GroupElement S = signerPublicKey.getBaseS();
    ...
    assertEquals(
        Z,
        Z_n,
        "The exponentiation over QRGroupN did not yield the same result as the BigInteger computation.");
    assertEquals(
        Z,
        Z_pq,
        "The CRT exponentiation over QRGroupPQ did not yield the same result as the BigInteger computation.");

    x_Z = CryptoUtilsFacade.computeRandomNumber(NumberConstants.TWO.getValue(), upperBound);

    // compute using BigIntegers
    BigInteger Ri = S.getValue().modPow(x_Z, modN);
  ...
  void computeCRTRandomMult() {
    ...
    BigInteger preMultiplier = CryptoUtilsFacade.createElementOfZNS(modN);
    BigInteger multiplier = preMultiplier.modPow(NumberConstants.TWO.getValue(), modN);
    ...
    Z = (S.getValue().multiply(multiplier)).mod(modN);
    ...
    assertEquals(
        Z,
        Z_n,
        "The computation in QRGroupN did not yield the same result as the BigInteger computation.");
    assertEquals(
        Z,
        Z_pq,
        "The CRT Computation in QRGroupPQ did not yield the same result as the BigInteger computation.");

    // Create a fresh likely-quadratic residue.
    preMultiplier = CryptoUtilsFacade.createElementOfZNS(modN);
    multiplier = preMultiplier.modPow(NumberConstants.TWO.getValue(), modN);
    multiplier_pq = new QRElementPQ((QRGroupPQ) qrGroupPQ, multiplier);
    multiplier_n = new QRElementN((QRGroupN) qrGroupN, multiplier);

    // compute using BigIntegers multiply
    BigInteger Ri = (S.getValue().multiply(multiplier)).mod(modN);
  ...
  //    log.info("crt t: " + EEAlgorithm.getT());
  //    log.info("crt modInverse: " + p.modInverse(q));
  //    QRElementPQ qr = new QRElementPQ(NumberConstants.TWO.getValue()); // TODO Needs access to
  // QRGroup.
